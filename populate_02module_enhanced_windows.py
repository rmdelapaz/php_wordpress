#!/usr/bin/env python3
"""
Enhanced Windows-compatible script to populate 02module files with content from 02module_old files.
This version better preserves formatting and handles special cases.
Works on Windows with WSL paths.
"""
...
def get_base_path():
    """
    Get the base path for the project, handling both Windows paths and WSL paths.
    """
    # Try different possible paths
    possible_paths = [
        # Direct WSL path access from Windows
        r'\\wsl$\Ubuntu\home\practicalace\projects\php_wordpress',
        r'\\wsl.localhost\Ubuntu\home\practicalace\projects\php_wordpress',
        # If running from within WSL
        '/home/practicalace/projects/php_wordpress',
        # If the folder is mapped to a Windows drive
        r'W:\home\practicalace\projects\php_wordpress',
        # Current directory (if running from project folder)
        '.'
    ]
    
    for path_str in possible_paths:
        path = Path(path_str)
        if path.exists() and (path / '02module').exists() and (path / '02module_old').exists():
            print(f"Found project at: {path}")
            return path
    
    # If not found, ask user
    print("Could not automatically find the project directory.")
    print("Please enter the full path to the php_wordpress folder:")
    print("Example for WSL: \\\\wsl$\\Ubuntu\\home\\practicalace\\projects\\php_wordpress")
    print("Example for Windows: C:\\Users\\YourName\\projects\\php_wordpress")
    ...
    raise Exception("Could not find valid project directory")
...
def extract_old_content(old_file_path):
    ...
    try:
        ...
        if main_match:
            ...
            print(f"  Warning: No <main> tag found in {old_file_path.name}")
            return None
            
    except Exception as e:
        print(f"  Error reading {old_file_path.name}: {e}")
        return None
...
def update_new_file(new_file_path, old_content):
    ...
    try:
        ...
        if match:
            ...
            print(f"  Warning: Could not find lesson-body pattern in {new_file_path.name}")
            return False
            
    except Exception as e:
        print(f"  Error updating {new_file_path.name}: {e}")
        return False
...
def update_lesson_title_and_meta(new_file_path, old_file_path):
    ...
    try:
        ...
        if title_match:
            ...
            # Extract the main part of the title (remove course suffix if exists)
...
def process_modules():
    ...
    print(f"\nFound {len(old_files)} HTML files in 02module_old")
    print("=" * 60)
...
def main():
    ...
    print("Enhanced Windows Version")
    print("=" * 60)
    
    # Create backup first
    print("\nStep 1: Creating backup of current 02module files...")
    ...
    print("\nStep 2: Starting content population process...")
    successful, failed, skipped = process_modules()
    
    if successful > 0:
        print("\nStep 3: Checking for Mermaid diagrams...")
    ...
    print("\n✨ Backup folder created for safety - you can restore if needed")
